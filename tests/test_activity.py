...
# Parametrize with single Solution
# Function-scoped parametrization of engine
class TestUnitActivity:
    ...
    @staticmethod
    @pytest.mark.parametrize(("salt_conc_units", "salt_conc", "engine"), [("mol/kg", 1e-11, "native")])
    ...
    @pytest.mark.parametrize("salt_conc", [1e-11])
...
class TestActivityPitzer:
    ...
    @staticmethod
    @pytest.fixture(name="expected_activity_coefficient")
    def fixture_expected_activity_coefficient(
        solution: Solution,
        salt: Salt,
        alphas: tuple[float, float],
    ) -> Quantity:
        molality = salt.get_effective_molality(solution.ionic_strength)  # type: ignore[no-untyped-call]
        param = solution.get_property(salt.formula, "model_parameters.activity_pitzer")
        assert param is not None
        return ac.get_activity_coefficient_pitzer(  # type: ignore[no-any-return, no-untyped-call]
    ...
    def test_should_return_activity_coefficient_of_major_salt_when_parameters_exist(
        solution: Solution, salt: Salt, expected_activity_coefficient: Quantity
    ) -> None:
        assert solution.get_activity_coefficient(salt.cation).m == expected_activity_coefficient.m
        assert solution.get_activity_coefficient(salt.anion).m == expected_activity_coefficient.m
