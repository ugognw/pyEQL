...
class TestComponents:
    @staticmethod
    @pytest.fixture(name="ions", params=[(), ("Na[+1]", "Cl[-1]"), ("Ca[+2]", "CO3[-2]")])
    def fixture_ions(request: pytest.FixtureRequest) -> tuple[str, ...]:
        ions: tuple[str, str] = request.param
        return ions

    @staticmethod
    @pytest.fixture(name="conc", params=["1 mol/L", "2 mol/L"])
    ...
    @staticmethod
    def test_should_return_floats(solution: Solution) -> None:
        components_are_floats = [isinstance(mol, float) for mol in solution.components.values()]
        assert all(components_are_floats)
